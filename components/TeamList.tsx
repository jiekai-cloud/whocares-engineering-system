...
const TeamList: React.FC<TeamListProps> = ({ members, departments, projects, onAddClick, onEditClick, onDeleteClick, currentUserRole }) => {
  ...
  return (
    <div className="p-4 lg:p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="bg-white p-1 rounded-xl border border-stone-200 flex gap-1 self-end sm:self-auto">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${viewMode === 'list' ? 'bg-stone-900 text-white' : 'text-stone-400 hover:text-stone-600'}`}
          >
            人員清單
          </button>
          <button
            onClick={() => setViewMode('chart')}
            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${viewMode === 'chart' ? 'bg-stone-900 text-white' : 'text-stone-400 hover:text-stone-600'}`}
          >
            組織佈局
          </button>
        </div>
      ...
      {viewMode === 'chart' ? (
        <OrgChart members={members} departments={departments} />
      ) : (
        <>
            ...
            <div className="divide-y divide-slate-100">
              {sortedMembers
                .filter(member => {
                  ...
                  // Add CEO back to purge list, BUT JK001 will be safe because it starts with JK
