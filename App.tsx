...
const App: React.FC = () => {
  ...
  const loadSystemData = useCallback(async (dept: SystemContext) => {
    ...
    const allPrefixes = ['', 'dept3_', 'dept4_'];

    const fetchUnified = async <T extends { id: string; updatedAt?: string; deletedAt?: string }>(key: string, fallback: T[]): Promise<T[]> => {
      const results = await Promise.all(allPrefixes.map(p => storageService.getItem<T[]>(`${p}${key}`, [])));
      // Start with fallback/default data and merge all found prefixed data
      return results.reduce((acc, current) => mergeData(acc, current), fallback);
    };

    const defaultProjects = dept === 'FirstDept' ? MOCK_PROJECTS : [];
    const initialProjects = await fetchUnified<Project>('bt_projects', defaultProjects);

    // Restoration Logic (Legacy FirstDept Support)
    if (dept === 'FirstDept' || dept === 'FourthDept') {
      const criticalRestorationIds = ['BNI2601001', 'BNI2601002', 'BNI2601004', 'OC2601005', 'JW2601003'];
      // ... (keeping existing restoration logic if needed, but fetchUnified handles most)
    }
    ...
    if (processedTeamData.length < originalCount) {
      ...
      const storageKey = `${prefix}bt_team`;
